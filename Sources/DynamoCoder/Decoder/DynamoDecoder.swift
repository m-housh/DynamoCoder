...
public struct DynamoDecoder {
    ...
    public func decode<T>(
        _ type: T.Type,
        from dict: DynamoAttributeDict
    ) throws -> T
        where T: Decodable
    {
        try _DynamoDecoder(referencing: dict).decode(type)
    }
    ...
    public func decode<T>(
        _ type: T.Type,
        from attribute: DynamoDB.AttributeValue
    ) throws -> T
        where T: Decodable
    {
...
class _DynamoDecoder: Decoder {
    ...
    var storage = DynamoDecoderStorage()
    ...
    func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> where Key : CodingKey {
        ...
        let container = DynamoKeyedDecoder<Key>(referencing: self, wrapping: topContainer)
    ...
    func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        ...
        return DynamoUnkeyedDecoder(decoder: self, container: topContainer, codingPath: codingPath)
    }
